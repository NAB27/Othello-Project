...
// structures
typedef struct securitee{
char nom[20];
char mot_de_passe[16];
struct securitee*suivant;}securitee;
...
void ajout_acces_securise(char nom[20],char mot_de_passe[16],securitee *list){
...
securitee*jou;
jou=(securitee*)malloc(sizeof(securitee));
strcpy(jou->nom,nom);
...
//deja enregistre
void connection(securitee*debut){
    ...
    securitee*utilisateur=(securitee*)malloc(sizeof(securitee));
...
int alphabeta (int combatant, int * latabl, int ply, int (* vff) (int, int *)) {
    ...
    int meuillermvt;
    int * mouvements;
    int * nvtable;int meuilleurch (int, int *, int, int (*) (int, int *));
    int mauvch (int, int *, int, int (*) (int, int *));
    mouvements = mouvementpossible(combatant, latabl);
    max = perdre - 1;
    for (i=1; i <= mouvements[0]; i++) {
        nvtable = enregistrer(latabl);tab = tab + 1;deplacer(mouvements[i], combatant, nvtable);
        ntm = suivantt(nvtable, combatant, 0);
        if (ntm == 0) {nvs = btweentwo(combatant, nvtable);
            if (nvs > 0) nvs = gagne;
            if (nvs < 0) nvs = perdre;}
        if (ntm == combatant)nvs = meuilleurch(combatant, nvtable, ply-1, vff);
        if (ntm == adv(combatant))nvs = mauvch(combatant, nvtable, ply-1, vff);
        if (nvs > max) {max = nvs;meuillermvt = mouvements[i];}
        free(nvtable);
    }
    free(mouvements);
    return(meuillermvt);
...
void game (int (* blplans) (int, int *),int (* whplans) (int, int *), int pf) {int * latabl;int combatant;latabl = debut();
    combatant = noir;
    do {if (combatant == noir) avoirmvt(blplans, noir, latabl, pf);else avoirmvt(whplans, blanc, latabl, pf);
        combatant = suivantt(latabl, combatant, pf);}
    while (combatant != 0);
    if (pf) {printf("Partie finie! resultat:\n");afficher(latabl);}}
...
int main (void) {
    ...
    affichage_scores(jliste);
        printf("vous voulez enregistrer un compte ?");
    if (getchar() == 'y'){char aaa[20];
        char aba[20];
        securitee *debbut;
        printf("entrer votre nom");
            ...
            scanf("%s",&aba);
        ajout_acces_securise(aaa,aba,debbut);
